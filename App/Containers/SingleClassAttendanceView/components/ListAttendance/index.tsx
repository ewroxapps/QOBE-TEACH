...
const ListAttendance = (props: any) => {
  const { data, index, studentDetails, setStudentDetails } = props;
  const togglePresentStatus = () => {
    setStudentDetails((prevDetails: Studentss[]) => {
      const updatedDetails = prevDetails.map((student, idx) =>
        idx === index - 1 ? { ...student, present: !student.present } : student
      );
    ...
    <View>
      <View style={styles.container}>
        ...
        <View style={styles.totalView}>
          <View
            style={
              studentDetails[index - 1]?.lateLeave === true
                ? styles.withlate
                : styles.withoutlate
            }>
            {studentDetails[index - 1]?.present === true ? (
          ...
          {studentDetails[index - 1]?.lateLeave ? (
